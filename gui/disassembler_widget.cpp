...
void DisassemblerWidget::saveASMFile(QString fileName) {
...
void DisassemblerWidget::printCell(QTextCursor &cursor, std::string text, int length, QTextCharFormat fmt) {
...
void DisassemblerWidget::printCell(QTextCursor &cursor, std::string text, int length) {
  int spclen=length-text.length();
  if (spclen<0) {
    spclen=0;
  }
